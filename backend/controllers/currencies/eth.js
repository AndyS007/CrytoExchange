...
// Get wallet balance
const getEthWalletBalance = async (address) => {
...
const sendETHTransaction = async (fromAddress, toAddress, amount) => {
    // Get privateKey
    ...
    try {
        ...
        return new Error(`Send ETH transaction failed. ${error.message}`);
    }

    return tx_data;
}

// Get all transactions of a ETH address
const getETHTransactionsOfAccount = async (address) => {
...
const getETHTransactionsOfId = async (tx_id) => {
